module.exports = function (_clone_and_set_option_defaults, _isArray, QRatio, extract) {
    ...
    module.dedupe = function dedupe(contains_dupes, options_p) {
        ...
        * @returns {Object[]} - array of unique items and the index/key of the used match in contains_dupes.
        */
        ...
        if (contains_dupes === [] || contains_dupes === {}) {
        ...
        for (var i in contains_dupes) {
            //_forEach(contains_dupes, function (value, key) {
            ...
            if (matches.length === 1) extractor.push([matches[0][0], matches[0][2]])
            else {
                // take longest, break tie by strign compare
                matches = matches.map(function (x) { return [x[0], x[2]] }).sort(function (a, b) {
                ...
                extractor.push(matches[0]);
            }

        }

        // shouldnt have to do extra hasOwnProperty check for extractor/uniqueKeys
        // faster way without additional dependencies?
        var uniqueItems = {};
        for (var e in extractor) {
            uniqueItems[processor(extractor[e][0])] = extractor[e];
        }

        var uniqueVals = [];

        for (var k in uniqueItems) {
            uniqueVals.push(uniqueItems[k]);
        }


        // extractor = _uniqBy(extractor, processor);
        if (uniqueVals.length === contains_dupes.length) return contains_dupes;
        else return uniqueVals;
    }
